	...
	pub fn call(
		...
		let res = match method {
			...
			InvokeMethod::Table(func) => {
				...
				let func_ref = if let Val::FuncRef(Some(func_ref)) =
					table.get(func).map_err(|_| Error::NoTableEntryWithIndex(func))?
				{
					func_ref
				} else {
			...
			InvokeMethod::TableWithWrapper { dispatcher_ref, func } => {
				...
				let func_ref = if let Val::FuncRef(Some(func_ref)) = table
					.get(dispatcher_ref)
