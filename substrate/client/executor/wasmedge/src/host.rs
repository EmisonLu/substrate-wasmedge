...
use std::sync::Arc;
use wasmedge_sdk::{Executor, FuncRef, Memory, Table};
...
impl<'a> sp_wasm_interface::FunctionContext for HostContext<'a> {
	fn read_memory_into(
		...
		let res = self.memory.read(address.into(), dest.len() as u32).map_err(|e| e.to_string())?;
		dest.copy_from_slice(&res);
		Ok(())
	}

	fn write_memory(&mut self, address: Pointer<u8>, data: &[u8]) -> sp_wasm_interface::Result<()> {
		self.memory.write(data.to_vec(), address.into()).map_err(|e| e.to_string())
	}
...
impl<'a> Sandbox for HostContext<'a> {
	fn memory_get(
		...
		if self.memory.write(buffer, buf_ptr.into()).is_err() {
	...
	fn memory_set(
		...
		let buffer = match self.memory.read(val_ptr.into(), len as u32) {
			Err(_) => return Ok(sandbox_env::ERR_OUT_OF_BOUNDS),
			Ok(buffer) => buffer,
		};
	...
	fn instance_new(
		...
		let dispatch_thunk = Arc::new({
			if let wasmedge_sdk::types::Val::FuncRef(Some(func_ref)) = self
				.table
				.as_ref()
